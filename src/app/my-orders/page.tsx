...
export default function MyOrdersPage() {
  ...
  const getStatusBadge = (status: Order['status']) => {
    switch (status) {
      case 'pending': return <span style={{ background: '#FFC107', color: 'black', padding: '4px 8px', borderRadius: '4px', fontSize: '0.8rem', fontWeight: 600 }}>{t('waiting_confirm')}</span>;
      case 'accepted': return <span style={{ background: 'var(--primary-color)', color: 'white', padding: '4px 8px', borderRadius: '4px', fontSize: '0.8rem', fontWeight: 600 }}>{t('preparing')}</span>;
      case 'completed': return <span style={{ background: '#4CAF50', color: 'white', padding: '4px 8px', borderRadius: '4px', fontSize: '0.8rem', fontWeight: 600 }}>{t('completed')}</span>;
      case 'rejected': return <span style={{ background: 'var(--accent-color)', color: 'white', padding: '4px 8px', borderRadius: '4px', fontSize: '0.8rem', fontWeight: 600 }}>{t('rejected')}</span>;
      case 'canceled': return <span style={{ background: '#999', color: 'white', padding: '4px 8px', borderRadius: '4px', fontSize: '0.8rem', fontWeight: 600 }}>{t('order_canceled')}</span>;
      default: return null;
    }
  };
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '12px' }}>
                <div>
                  ...
                  <h3 style={{ fontSize: '1.1rem', fontWeight: 600 }}>
                    Shop ID: {order.shopId.substring(0,8)}...
                  </h3>
                </div>
